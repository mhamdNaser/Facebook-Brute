...
def generate_passwords(user):
    base_words = ['password', '123456', 'facebook', 'qwerty', 'admin']
    username_parts = [user]
    if '@' in user:
        username_parts.append(user.split('@')[0])
    generated = []

    for word in base_words + username_parts:
        for i in range(100):
            pwd = f"{word}{i}"
...
def args():
    ...
    parser.add_argument('--generate-passwords', action='store_true', help='Generate Passwords Dynamically')
...
""")
    ...
    elif args and args.generate_passwords:
